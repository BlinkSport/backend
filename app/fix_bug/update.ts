...
export default class HandleSessionMembersController {
  ...
  async acceptNewMember({ request, auth, response }: HttpContext) {
    ...
    try {
      ...
      const sessionMember = SessionMember.query()
        .where('sessionId', sessionId)
        .andWhere('userId', userId)
        .andWhere('isAccepted', false)

      // return response.json(sessionMember)

      // Accepter la demande
      // sessionMember.isAccepted = true // Accepter la demande
      // await sessionMember.save()

      await sessionMember.update({ isAccepted: true })
